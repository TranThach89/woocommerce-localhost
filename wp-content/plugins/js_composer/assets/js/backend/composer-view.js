...
(function ($) {
                ...
                _.each(settings.params, function (p) {
                    ...
                    if ($admin_label.length) {
                        if (_.isObject(p.value) && !_.isArray(p.value) && p.type == 'checkbox') {
                            var inverted_value = _.invert(p.value);
    ...
    var ElementBlockView = vc.element_block_view = Backbone.View.extend({
        ...
        template:_.template($('#wpb-elements-list-modal-template').html()),
        data_saved:false,
        events:{
        ...
        render:function () {
            ...
            if (tag !== false && !_.isUndefined(vc.map[tag].allowed_container_element)) {
        ...
        createElement:function (e) {
            ...
            if (this.container.$content.is('#visual_composer_content')) {
                var row = model = Shortcodes.create({shortcode:'vc_row'}),
                ...
                if ($button.data('element') == 'vc_row') {
                    var row = model = Shortcodes.create({
    ...
    var SettingsView = Backbone.View.extend({
        tagName:'div',
        className:'wpb_bootstrap_modals',
        template:_.template($('#wpb-element-settings-modal-template').html()),
        data_changed:false,
        ...
        askSaveData:function () {
            if (this.data_saved || (!this.data_saved && confirm(window.i18nLocale.if_close_data_lost))) {
                this._killEditor();
                return true;
            }
            return false;
        ...
        close:function () {
            if (this.data_saved || confirm(window.i18nLocale.if_close_data_lost)) {
                this.data_saved = true;
                this._killEditor();
                this.$el.modal('hide');
            }
    ...
    var VisualComposer = Backbone.View.extend({
        ...
        addShortcode:function (model) {
            var view = this.getView(model),
                ...
                $element_to_add = model.get('parent_id') !== false
                    ...
                    :
                    this.$content;
            view.use_default_content = model.get('cloned') === true ? false : true;
            this.views[model.id] = view;
        ...
        saveTemplate:function (e) {
            ...
            if (_.isString(name) && name.length) {
                var shortcodes = vc.storage.getContent(),
                    data = {
        ...
        setSortable:function () {
            ...
            $('.wpb_column_container').sortable({
                ...
                over:function (event, ui) {
                    var parent_tag = ui.placeholder.closest('[data-element_type]').data('element_type'),
        ...
        switchComposer:function (e) {
            ...
            if (this.status == 'shown') {
                this.$switchButton.text(window.i18nLocale.main_button_title);
                this.close();
                this.status = 'closed';
            } else {
