...
class Promolizers_Cart_Data {
    function __construct(){
        ...
        // Add Setup fee
        add_filter( 'woocommerce_before_calculate_totals', array( $this, 'add_setup_fee'));
    ...
    public function woocommerce_add_cart_item_data($cart_item_meta, $product_id){
        global $wpdb;
        //Table Name
        $product_colorsTable         = $wpdb->prefix."product_colors";
        ...
        // Add imprint type variable
        if( isset( $_POST['imprintType'] ) && !empty( $_POST['imprintType'] ) ){
            $imprintType = $_POST['imprintType'];
            $imprintTypeNameObject = $wpdb->get_row("SELECT * FROM {$imprint_typesTable} WHERE `type_slug` = '{$imprintType}'");
            $data['imprint_type'] = array(
                'name' => 'Imprint Types',
                'value' => $imprintTypeNameObject->type_name,
                'price' => 0,
            );
            $cart_item_meta['setup'] = $imprintTypeNameObject->setup_charge;
        }
        // Add imprint location variable
        if( isset( $_POST['imprintLocation'] ) && !empty( $_POST['imprintLocation'] ) ){
            ...
            $data['imprint_location'] = array(
                ...
                'value' => str_replace("Shirts - ","",implode(", ",$imprintLocationName)),
                'price' => 0,
            );
        }
        // Add product color variable
        if( isset($_POST['color'] ) && !empty( $_POST['color'] ) ){
            ...
            $product_colorName = $wpdb->get_col("SELECT `color_name` FROM {$product_colorsTable} WHERE `color_slug` IN ({$productColors})");
            $data['imprint_color'] = array(
                'name' => 'Imprint Colors',
                'value' => implode(", ",$product_colorName),
                'price' => 0,
            );
        }
        ...
        if( isset( $_POST['step3size'] ) ){
            ...
            if(is_array($size)){
                foreach($size as $colorname=>$colorValue){
                    $sizeString .= $this->get_product_color_name($colorname).": ";
        ...
        if(!empty($data['_izw_override_shipping']) && $data['_izw_override_shipping'] ){
            ...
            $checkShipping = get_option('izw_flat_rate_shipping');
            $ShippingAmount = get_option('izw_flat_rate_amount');
            if($checkShipping && (float)$ShippingAmount > 0){
                $cart_item_meta['izw_product_shipping'] = $ShippingAmount;
            }
        }
    ...
    public function get_cart_item_from_session( $cart_item, $values ) {
        ...
        if ( ! empty( $values['setup'] ) ) {
            $cart_item['setup'] = $values['setup'];
        }
    ...
    function add_to_cart_validation($passed, $product_id, $qty){
        ...
        if( isset($_FILES['browse'] )  ){
            ...
            if(!$passed){
                $woocommerce->add_error( sprintf( __('Please select design Art/Logo ', 'woocommerce')) );
            }
        }else{
            $passed = false;
            $woocommerce->add_error( sprintf( __('Please select design Art/Logo ', 'woocommerce')) );
     ...
     * Add Setup Fee
     */
    function add_setup_fee(){
        $settup = 0;
        foreach(WC()->cart->get_cart() as $rows){
            $settup += (float)$rows['setup'];
        }
        if( $settup > 0 )
            WC()->cart->add_fee( __('Setup', 'woocommerce'), $settup );
    }
    ...
    function init(){
        global $woocommerce;
        if(!is_admin()){
            $total_shipping = get_option('izw_shipping_order_total',true);
            $izw_check_order_total = get_option('izw_check_order_total',true);
            $total_shipping = (float)$total_shipping;
            $total_cart = (float)$woocommerce->cart->subtotal;
            // Free shipping for all total cart lt order total in admin
            if(!empty($izw_check_order_total) && $izw_check_order_total=='1' && $total_cart >= $total_shipping){
                ...
                add_filter( 'woocommerce_package_rates', function($available_methods){
                    return array( 'izw_promo_shipping' => $available_methods['izw_promo_shipping'] );
                } );
            }
        }
    }
    function get_product_color_name($slug, $by='color_slug'){
        global $wpdb;
        $product_colorsTable     = $wpdb->prefix."product_colors";
        return $wpdb->get_var("SELECT `color_name` FROM {$product_colorsTable} WHERE `".$by."` = '{$slug}'");
    }
}
