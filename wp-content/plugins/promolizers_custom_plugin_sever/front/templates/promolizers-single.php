...
<div class="product_color_page">
    <form class="cart" id="addtoCartBox" method="post" action="" enctype="multipart/form-data">
        ...
        <div class="izw_box_single" style="display: table;">
            ...
            <fieldset>
                        ...
                        <tr class="msrp-row">
                            ...
                            if(!empty($izwpsp['DT_izw_msrp'])  && (is_array($izwpsp['DT_izw_msrp']))){
                                foreach($izwpsp['DT_izw_msrp'] as $key=>$values): ?>
                                    <td><?php echo $values[$izwpsp['DT_izw_imprint_type_default']]; ?></td>
                                <?php endforeach;}
                            else{
                                _e( "Please choose", __TEXTDOMAIN__ );
                            }?>
                ...
                <div class="quantity_product">
                    <div class="required-product-addon product-addon product-addon-total-quantity">
                        ...
                        <div class="form-row-wide">
                            <p>
                                ...
                                <input name="quantity" id="inputAmount" value="<?php echo $min_izwpsp;?>" type="hidden" />
                            </p>
                            <div id="qty-slider" class="addon addon-total-quantity"></div>
                            <input autocomplete="off" id="qty-text" name="imprint_quantity" class="addon" type="text" value="<?php echo $min_izwpsp;?>" />
                        </div>
                    </div>
                    <div id="ProductColor1" class="required-product-addon product-addon product-addon-color-group">
                        ...
                        <p class="description"><i><?php _e( "You may select up to 3 product colors", __TEXTDOMAIN__ ); ?></i></p>
                            ...
                            <p class="form-row form-row-wide">
                                ...
                                if(!empty($izwpsp['product_colors'])  && (is_array($izwpsp['product_colors']))){
                                    foreach ($izwpsp['product_colors'] as $key):
                                        ...
                                        ?>
                                        <span title="<?php echo $key->color_name;?>" id="<?php echo $key->color_slug;?>" class="<?php echo $class; ?>" data-string="<?php echo $key->color_slug;?>" style="background:<?php echo $key->hex_code1;?>"></span>
                                    <?php endforeach;}
                                else{ ?>
            ...
            <fieldset>
                <div class="required-product-addon product-addon product-addon-imprint-type">
                    ...
                    <div class="embroidery_imprint_type">
                                                <span id="it-embroidery" class="addon addon-imprint-type <?php if($type->ID == $izwpsp['DT_izw_imprint_type_default']) echo "selected";?>" title="<?php echo $type->type_name;?>" data-string="<?php echo $type->type_slug;?>" data-color="<?php echo $type->max_color;?>" data-location="<?php echo $type->max_location;?>">
                    ...
                    <?php if($type->ID == $izwpsp['DT_izw_imprint_type_default']){?>
                ...
                <?php if($izwpsp['check_location'] && !empty($izwpsp['imprint_locations'])  && (is_array($izwpsp['imprint_locations']))): ?>
                        ...
                        <div class="imprint_type">
                            ...
                            foreach ($izwpsp['imprint_locations'] as $location):?>
                                <div class="form-row form-row-wides">
                                                <span id="il-lt-sleeve" class="addon addon-imprint-location" title="<?php echo get_substr($location->location_name);?>" data-string="<?php echo $location->location_slug;?>">
                                                    <img src="<?php echo $location->image;?>"/>
                                                </span>
                                    <p><?php echo get_substr($location->location_name);?></p>
                                </div>
                            <?php endforeach;
                            ?>
                        </div>
        ...
        <div class="izw_sidebar">
            ...
            <div class="sub_total">
                <div class="sub_total_table">
                    <?php if(check_onsale_product()) $min = $izwpsp['DT_izw_sale'][0][$izwpsp['DT_izw_imprint_type_default']];else $min = $izwpsp['DT_izw_price'][0][$izwpsp['DT_izw_imprint_type_default']];?>
                    <h4><?php _e("Sub Total", __TEXTDOMAIN__);?><span class="totalstep1">
                                        <?php echo "$",number_format(($min * $min_izwpsp),2);?></span></h4>
                    <p class="pricestep1"><?php echo "$",$min," x ",$min_izwpsp;?></p>
                    <h4><?php _e("Saving", __TEXTDOMAIN__);?>
                        <span class="savingstep1">
                                        <?php
                                        $saving = ($izwpsp['DT_izw_msrp'][0][$izwpsp['DT_izw_imprint_type_default']] - $min) * $min_izwpsp;
                                        if($saving <= 0){
                                            $saving = 0;
                                        }
                    ...
                    <h4><?php _e("Setup", __TEXTDOMAIN__);?><span class="setup">
                            ...
                            <?php echo "$",number_format($setup, 2); ?></span></h4>
                    <h4 class="romoship">
                        <?php if(!empty($izwpsp['DT_izw_override_shipping']) && $izwpsp['DT_izw_override_shipping'] == '1'): ?>
                        <label>Shipping</label>
                        <?php else:?>
                            <img src="<?php echo __PLUGINURL__; ?>assets/front/images/romoship.png">
                        <?php endif; ?>
                                    <span>
                                         <?php
                                         if(!check_waive_shipping(($min * $min_izwpsp) + $setup)){
                                             ...
                                             elseif(!empty($izwpsp['DT_izw_flat_rate_shipping']) && $izwpsp['DT_izw_flat_rate_shipping'] == '1'):
                                                 $shipping = $izwpsp['DT_izw_flat_rate_amount'];
                                                 echo "$",number_format($shipping,2);
                                             else :
                                                 $shipping = 0;echo "Free";
                                             endif;
                                         }else {$shipping = 0;echo "Free";}?>
                                    </span>
                        <input type="hidden" value="<?php echo $shipping; ?>" name="romoship"/>
                    </h4>
                    <!--<p style="margin: 0px 9%; width: 80%; border: 1px solid rgb(231, 231, 231);"></p>-->
                    <p style="background-color: rgba(0, 0, 0, 0.2); height: 1px;"></p>
