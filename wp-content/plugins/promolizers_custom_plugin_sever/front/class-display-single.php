...
class IZW_Display_Single {
    function __construct(){
        ...
        add_action( 'woocommerce_product_meta_start', array( $this, 'add_item_meta_front_end') );
        //add_action( 'woocommerce_product_add_to_cart_url', array( $this, 'change_url_add_to_cart') );
        //add_action( 'woocommerce_product_add_to_cart_text', array( $this, 'change_text_add_to_cart') );
        //add_action( 'woocommerce_product_single_add_to_cart_text', array( $this, 'change_text_add_to_cart') );
    }
    ...
    function izw_define_products(){
        ...
        $DT_izw_imprint_type_default               = get_post_meta( $product_id, '_izw_imprint_type_default', true ) ;
        if(!empty($DT_izw_imprint_type_default)){
            $izwpsp['DT_izw_imprint_type_default']               = unserialize( get_post_meta( $product_id, '_izw_imprint_type_default', true ) );
        }

        $options = array(
            'izw_flat_rate_shipping',
            'izw_flat_rate_amount',
            'izw_check_order_total',
            'izw_shipping_order_total'
        );
        foreach( $options as $key):
            $check = get_option( $key , true);
            if(!empty($check)){
                $izwpsp['DT_'.$key]         =      get_option( $key , true);
            }
        endforeach;
        $izwpsp = apply_filters( 'izw_promolizers_single_product',$izwpsp,$post);
    }
    function add_item_meta_front_end(){
        global $post;
        $item = get_post_meta( $post->ID, '_izw_item', true);
        if(!empty($item)){
            ?>
            <label class="_izw_item_wrapper"><?php _e( "Item", "woocommerce" ); ?> #:</label>
            <span class="_izw_item">
                <?php echo unserialize($item); ?>
            </span>
            <?php
        }
    }
    function change_url_add_to_cart($url){
        global $post;
        $enable = get_post_meta($post->ID, '_izw_enable', true);
        if($enable){
            $url = get_permalink($post->ID);
        }
        return $url;
    }
    function change_text_add_to_cart($text){
        global $post;
        $enable = get_post_meta($post->ID, '_izw_enable', true);
        if($enable){
