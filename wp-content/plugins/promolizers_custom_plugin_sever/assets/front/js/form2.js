...
var selColors_name = [];//luu gia tri color dc chon o buoc 1
...
$jQ("form.cart").on("fs_shown", function(event, fsIndex){
    ...
    selColors_name = [];//save number title select color step1
    $jQ("#ProductColor1 .addon-color-swatch.selected").each(function(index){
        selColors_name.push($jQ(this).attr("title"));
    });

    if (fsIndex && fsIndex == 1){
        reset_html(fsIndex);
        $jQ(".info_step1").append('<p>Product color(s):' + selColors_name.toString() + '</p><p>Quantily:' + sumquaty + '</p>');
...
//  STEP3
    if (fsIndex && fsIndex == 2){
        reset_html(fsIndex);
        imLoca = $jQ('.addon-imprint-location.selected').map(function(){return $jQ(this).attr("title");}).get();
        $jQ(".info_step2").append('<p>Imprint Type: ' + $jQ('.addon-imprint-type.selected').attr("title") + '</p><p>Imprint Location(s): ' + imLoca.toString() + "</p>");
    }
...
//  STEP 4
    if(fsIndex==3){
        ...
        reset_html(fsIndex);
        $jQ(".info_step3").append(info_step3_html(selColors));
        //call html step4
        step4 = html_step4();
        ...
        case "embroidery":
            $jQ($ColorIDContent + ' .vehicle_check_embroidery').click(function(){
                if($jQ($ColorIDContent + ' .vehicle_check_embroidery').is(":checked")){
                    ...
                    jquery_iris(jqchoice,jqclick,jqinput,$selColors,1);
                }else{
            ...
            $jQ($ColorIDContent + ' .vehicle_check').click(function(){
                if($jQ($ColorIDContent + ' .vehicle_check').is(":checked")){
                    ...
                    <div class="checkedcolor">\
                        <div class="addcolor">\
                            <div class="addcolor1">\
                                <div class="input"><input type="radio" value="Pantone" name="checkedcolor[' + $selColors + '][1]" checked="true" />Pantone/PMS</div>\
                                <div class="input"><input type="radio" value="RGB" name="checkedcolor[' + $selColors + '][1]">RGB color</div>\
                                <div class="input"><input type="radio" value="Hex" name="checkedcolor[' + $selColors + '][1]">Hex color</div>\
                                <div class="input"><input type="radio" value="Custom" name="checkedcolor[' + $selColors + '][1]">Custom</div>\
    ...
    if(save_num[$selColors] + $countadd[$selColors] < $setcolor){
        if(varold==""){
            ...
            $jQ('#ColorIDContent_' + $selColors + ' .addcolor').append('<div class="addcolor'+ $countadd[$selColors] +'">' +
                    '<div class="input"><input type="radio" name="checkedcolor[' + $selColors + "][" + $countadd[$selColors] + ']" value="Pantone" checked="checked" />Pantone/PMS</div>' +
                    '<div class="input"><input type="radio" name="checkedcolor[' + $selColors + "][" + $countadd[$selColors] + ']" value="RGB">RGB color</div>' +
                    '<div class="input"><input type="radio" name="checkedcolor[' + $selColors + "][" + $countadd[$selColors] + ']" value="Hex">Hex color</div>' +
                    '<div class="input"><input type="radio" name="checkedcolor[' + $selColors + "][" + $countadd[$selColors] + ']" value="Custom">Custom</div>' +
...
function step4_normal(){
    return '<h3 class="addon-name">Design <abbr class="required" title="required">*</abbr></h3>\
        <p class="form-row form-row-wide">\
            <label for="browse">Upload Art/Logo:</label></br>\
        ...
        <p class="form-row additional">\
            <label for="additional">Additional Information:</label>\
            <textarea id="additional" name="additional" style="width: 300px;" placeholder="Please be as descriptive as possible..."></textarea>\
        </p>';
}
...
function info_step3_html(selColors){
    var $return="<span class='imprint'>Imprint Color(s): </span><div class='color'>";
    for (i = 0; i < selColors.length; i++){
        var color = '';
        color = $jQ("#ColorIDContent_" + selColors[i] + " .addon-color-swatch.selected").map(function(){return $jQ(this).attr("title");}).get();
        $return+= '(' + selColors_name[i] + ') - ' + color.toString();
        if(save_array_vehicle[selColors[i]] != '') $return+= "," + save_array_vehicle[selColors[i]].toString();
    }
    $return+= "</div><span class='imprint'>Sizes: </span>";
    for (i = 0; i < selColors.length; i++){
        $return+= '<div>(' + selColors_name[i] + ') <div class="color"><p>S - ' + $jQ("#text-small-" + selColors[i]).val() + "</p>" +
                "<p>M - " + $jQ("#text-medium-" + selColors[i]).val() + "</p>" +
                "<p>L - " + $jQ("#text-large-" + selColors[i]).val() + "</p>" +
                "<p>XL - " + $jQ("#text-xlarge-" + selColors[i]).val() + "</p></div></div>";
    }
    return $return;
    //currentSelColors
}
