...
/**
 * Check Waive shipping for orders over
 * @param float $total
 * @return bool
 */
function check_waive_shipping($total = 1000){
    global $izwpsp;
    $return = false;
    if(!empty($izwpsp['DT_izw_check_order_total']) && $izwpsp['DT_izw_check_order_total'] == '1'):
        if(floatval($total) >= floatval($izwpsp['DT_izw_shipping_order_total'])) $return = true;
            else $return = false;
    endif;
    return $return;
}
...
function get_substr($string){
    ...
    foreach($findme as $key){
		if(strpos($string,$key)) return substr($string,strpos($string,$key) + strlen($key));
	}
}