 ...
 * Time: 8:56 AM
 */
class IZW_Custom_Product_Data2{
    function __construct(){
        ...
        add_action( 'woocommerce_process_product_meta', array( $this, 'remove_msrp' ),1,2 );
    ...
    function remove_msrp($post_id, $post){
        unset($_POST['_msrp_price']);
    }
    /**
    ...
    function custom_product_data($tabData){
        ...
        $tabData['izw_custom'] = array(
            'label'  => __( 'Custom2', 'woocommerce' ),
            'target' => 'custom_product_data2',
            'class'  => array('show_if_simple2'),
        );
        return $tabData;
    }
    ...
    function custom_product_data_panel(){
        ...
        <p class="form-field">
            ...
            <input type="text" class="short" name="_izw_supplier" id="_izw_supplier" value="<?php echo !empty($data['_izw_supplier']) ? $data['_izw_supplier'] : ''; ?>" placeholder="" maxlength="3"> -
            <input type="text" class="short" name="_izw_sku" id="_izw_sku" value="<?php echo !empty($data['_izw_sku']) ? $data['_izw_sku'] : ''; ?>" placeholder="">
        </p>
        <p class="form-field">
        ...
        <div class="dynamic-pricing-table">
            <table id="dynamic-pricing" class="dynamic-pricing" width="100%" cellspacing="0" cellpadding="0">
                <?php if($pagenow != "post-new.php"){?>
                    ...
                    <tr class="quantity">
                        <?php $i=0; $min = !empty($data['_izw_min']) ? min($data['_izw_min']) : 0; foreach($data['_izw_max'] as $key=>$values): ?>
                            <td>
                                <?php if($i == 0): ?>
                                    <?php _e( "Min Qty", __TEXTDOMAIN__); ?>: <input type="text" name="_izw_min[<?php echo $i; ?>]" value="<?php echo $min; ?>"/>
                                <?php endif; ?>
                                <?php _e( "Max Qty", __TEXTDOMAIN__); ?>: <input type="text" name="_izw_max[<?php echo $key; ?>]" value="<?php echo $values ? $values : ''; ?>"/>
                            </td>
                            <?php $i++; endforeach; ?>
                        <td></td>
                    </tr>
                    ...
                    <?php $i=1; if(!empty($imprintTypes)) foreach($imprintTypes as $type):
                        ...
                        <tr class="msrp ip_<?php echo $type->ID, " {$class}"; ?>" data-imprintID="<?php echo $type->ID; ?>">
                            <?php foreach($data['_izw_msrp'] as $key=>$values):?>
                                <td>
                                    <?php _e( "MSRP", __TEXTDOMAIN__); ?><input type="text" name="_izw_msrp[<?php echo $key; ?>][<?php echo $type->ID; ?>]" value="<?php echo $values[$type->ID] ? $values[$type->ID] : '0.00'; ?>"/>
                                </td>
                            <?php endforeach;?>
                            <td></td>
                        </tr>
                        <tr class="price ip_<?php echo $type->ID, " {$class}"; ?>" data-imprintID="<?php echo $type->ID; ?>">
                            <?php foreach($data['_izw_price'] as $key=>$values):?>
                                <td>
                                    <?php _e( "Price", __TEXTDOMAIN__); ?>: <input type="text" name="_izw_price[<?php echo $key; ?>][<?php echo $type->ID; ?>]" value="<?php echo $values[$type->ID] ? $values[$type->ID] : '0.00'; ?>"/>
                                </td>
                            <?php endforeach;?>
                            <td></td>
                        </tr>
                        <tr class="sale ip_<?php echo $type->ID, $class == 'active' ? " ".$class." " : ' ', "hide"; ?>" data-imprintID="<?php echo $type->ID; ?>">
                            <?php foreach($data['_izw_sale'] as $key=>$values):?>
                                <td>
                                    <?php _e( "Sale", __TEXTDOMAIN__); ?>: <input type="text" name="_izw_sale[<?php echo $key; ?>][<?php echo $type->ID; ?>]" value="<?php echo $values[$type->ID] ? $values[$type->ID] : '0.00'; ?>"/>
                                </td>
                            <?php endforeach;?>
                            <td></td>
                        </tr>
                    ...
                    <tr class="shipping hide">
                        <?php $i=0; foreach($data['_izw_shipping'] as $rows): ?>
                            <td>
                                <?php _e( "Shipping", __TEXTDOMAIN__); ?>: <input type="text" name="_izw_shipping[<?php echo $i; ?>]" value="<?php echo $rows ? $rows : '0.00'; ?>"/>
                            </td>
                            <?php $i++; endforeach; ?>
                        <td></td>
                    </tr>
                    ...
                    <script type="text/javascript">
                        jQuery(document).ready(function ($){
                            $("#addColumn").click(function (){
                                ...
                                var shippingHtml = '<td>' +
                                    'Shipping: <input type="text" name="_izw_shipping['+columnNumber+']" value="0.00"/>' +
                                    '</td>';
                                ...
                                $('table.dynamic-pricing').find('tr').each(function(){
                                    ...
                                    if($(this).hasClass("msrp")){
                                        $(this).find('td').eq(countColum-2).after('<td>' +
                                            'MSRP: <input type="text" name="_izw_msrp['+columnNumber+']['+$(this).attr('data-imprintID')+']" value="0.00"/>' +
                                            '</td>');
                                    }
                                    if($(this).hasClass("price")){
                                        $(this).find('td').eq(countColum-2).after('<td>' +
                                            'Price: <input type="text" name="_izw_price['+columnNumber+']['+$(this).attr('data-imprintID')+']" value="0.00"/>' +
                                            '</td>');
                                    }
                                    if($(this).hasClass("sale")){
                                        $(this).find('td').eq(countColum-2).after('<td>' +
                                            'Sale: <input type="text" name="_izw_sale['+columnNumber+']['+$(this).attr('data-imprintID')+']" value="0.00"/>' +
                                            '</td>');
                                    }
                                });
    ...
    function save( $post_id, $post ){
        ...
        update_post_meta( $post_id, '_sku',$data['_izw_supplier']."-". $data['_izw_sku'] );
        if(is_array($data['_izw_price'])){
            update_post_meta( $post_id, '_regular_price', $data['_izw_price'][$countColumn-1][$data['_izw_imprint_type_default']] );
            update_post_meta( $post_id, '_price', $data['_izw_price'][$countColumn-1][$data['_izw_imprint_type_default']] );
        }
        if(is_array($data['_izw_msrp'])){
            update_post_meta( $post_id, '_msrp_price', $data['_izw_msrp'][$countColumn-1][$data['_izw_imprint_type_default']] );
        }else{
            delete_post_meta( $post_id, '_msrp_price' );
        }
        if( !empty($data['_izw_onsale']) && $data['_izw_onsale'] && is_array($data['_izw_sale'])){
            update_post_meta( $post_id, '_sale_price', $data['_izw_sale'][$countColumn-1][$data['_izw_imprint_type_default']] );
            update_post_meta( $post_id, '_price', $data['_izw_sale'][$countColumn-1][$data['_izw_imprint_type_default']] );
        }else{
        ...
        if(isset($_POST['IPID']) && !empty($_POST['IPID'])){
            ...
            foreach($data as $key=>$values){
                ...
                if( !is_array($data[$key]) && empty($data[$key]) &&
                    ...
                    $data[$key] = array();
                    for($i=0; $i<=4; $i++){
                        foreach($data['imprintTypes'] as $rows){
                            $data[$key][$i][$rows->ID] = '0.00';
                        }
                    }
                }elseif(!is_array($data[$key]) && empty($data[$key]) && $key == '_izw_shipping'){
            ...
            <tr class="quantity">
                <?php $i=0; $min = !empty($data['_izw_min']) ? min($data['_izw_min']) : ''; foreach($data['_izw_max'] as $key=>$values): ?>
                    <td>
                        <?php if($i == 0): ?>
                            <?php _e( "Min Qty", __TEXTDOMAIN__); ?>: <input type="text" name="_izw_min[<?php echo $i; ?>]" value="<?php echo $min; ?>"/>
                        <?php endif; ?>
            ...
            <tbody>
            <?php $i=1; foreach($data['imprintTypes'] as $type):
                $class = 'hide';
                if( empty( $data['_izw_imprint_type_dnmprc'] ) && $i == 1 ) $class = 'active';
                ?>
                <tr class="msrp ip_<?php echo $type->ID, " {$class}"; ?>" data-imprintID="<?php echo $type->ID; ?>">
                    <?php foreach($data['_izw_msrp'] as $key=>$values):?>
                        <td>
                            <?php _e( "MSRP", __TEXTDOMAIN__); ?><input type="text" name="_izw_msrp[<?php echo $key; ?>][<?php echo $type->ID; ?>]" value="<?php echo !empty($values[$type->ID]) ? $values[$type->ID] : '0.00'; ?>"/>
                        </td>
                    <?php endforeach;?>
                    <td></td>
                </tr>
                <tr class="price ip_<?php echo $type->ID, " {$class}"; ?>" data-imprintID="<?php echo $type->ID; ?>">
                    <?php foreach($data['_izw_price'] as $key=>$values):?>
                        <td>
                            <?php _e( "Price", __TEXTDOMAIN__); ?>: <input type="text" name="_izw_price[<?php echo $key; ?>][<?php echo $type->ID; ?>]" value="<?php echo !empty($values[$type->ID]) ? $values[$type->ID] : '0.00'; ?>"/>
                        </td>
                    <?php endforeach;?>
                    <td></td>
                </tr>
                <tr class="sale ip_<?php echo $type->ID, $class == 'active' ? " ".$class." " : ' ', "hide"; ?>" data-imprintID="<?php echo $type->ID; ?>">
                    <?php foreach($data['_izw_sale'] as $key=>$values):?>
                        <td>
                            <?php _e( "Sale", __TEXTDOMAIN__); ?>: <input type="text" name="_izw_sale[<?php echo $key; ?>][<?php echo $type->ID; ?>]" value="<?php echo !empty($values[$type->ID]) ? $values[$type->ID] : '0.00'; ?>"/>
                        </td>
                    <?php endforeach;?>
                    <td></td>
                </tr>
